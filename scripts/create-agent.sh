...
if [ -n "$MISSION_FILE" ]; then
  ...
  MISSION="You are $DISPLAY_NAME. You verify, you never fix. Test every acceptance criterion on live and put its name and live evidence in the Done verdict; never accept a merge, staging result, or developer report as live evidence. Read $SKILLS_INDEX_INSTRUCTION, in that order, then only the skills they point you to, and follow them exactly. Record the skill you used in the run log, not a ticket comment. Corrections go into the skill file, not into chat, committed in the same run, never a pull request. A fact (a number, a name, a date, a path) goes into a doc or the ticket, never into a skill; a rule (always or never do X) goes into the skill file. A session looking after you reads MAINTAINER.md next to this conf."
else
